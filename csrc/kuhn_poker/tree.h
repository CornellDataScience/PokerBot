...
namespace kuhn_poker
{
                                                   ...
                                                   int max_depth)
  {
    ...
    for (int node_id = 0; node_id < static_cast<int>(nodes.size()) &&
         ...
         ++node_id)
    {
      const auto [start, end] = game.get_action_list(nodes[node_id].state);
  ...
  struct ChildrenActionIt
  {
    ...
    State begin() const
    {
      return State(node.children_begin, game.get_action_list(node.state).first);
    }
    State end() const
    {
      return State(node.children_end, game.get_action_list(node.state).second);
    }
  };
