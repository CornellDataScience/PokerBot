...
struct PartialPublicState {
  ...
  bool operator==(const PartialPublicState& state) const {
    ...
    && relative_pot == state.relative_pot && starting_player == state.starting_player;
  }
};
...
class Game {
 public:
  ...
  bool is_terminal(const PartialPublicState& state) const {
    return state.last_action % 2== 0 || (state.last_action == 1 && state.player_id == 1-state.starting_player;
  }
